onload = function(){
	...
	require(['jquery','base','jfa'], function($,Base,Jfa) {
		var Start = (function(){
			var _obj = {};
			return {
				getCity : function(arr){
					$('select.city').html(function(){
						var htm = '<option value="all" >全部</option>';
						$.each(arr.split(','),function(m,n){
							if(n.length>0) htm += '<option>'+n+'</option>';
						})
						return htm;
					})
				},
				...
				getData : function(){
					$.ajax({
						...
						success : function(back){
							if(back.state=='ok'){
								log(back);
								$('dl.number').html(function(){
									var htm  = '';
									$.each(back.data,function(m,n){
										htm += '<dt>'+n.numb+' '+n.province+' '+n.city+'</dt>';
									})
									return htm;
								})
							}
				...
				init : function(){
					...
					$('a.logout').click(function(){
						Base.user.logout(function(back){
					...
					$('a.quary').click(function(){
						_obj.getData();
					})
					$('a.add').click(function(){
						var numb = $('input.val').val().trim();
						if(numb && numb.length==11){
							$.ajax({
								url : '/mb/add',
								data : {numb:numb},
								success : function(back){
